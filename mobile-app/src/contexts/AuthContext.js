...
// import { USER_TOKEN_KEY, USER_DATA_KEY } from '../constants/appConstants';
...
//       return { ...prevState, userToken: action.token, userData: action.userData, isLoading: false };
//     case 'SIGN_OUT':
//       return { ...prevState, userToken: null, userData: null, isLoading: false };
//     case 'SET_USER_DATA': // For updating user data after initial login
//       return { ...prevState, userData: action.userData };
//     default:
//       return prevState;
//   }
// };
...
//     isLoading: true, // Start with loading true until token is restored
//     userToken: null,
//     userData: null,
//   });

//   // Effect to restore token and user data from AsyncStorage on app start.
//   useEffect(() => {
...
//         // Consider what to do here - maybe dispatch SIGN_OUT or an error state
//       }
...
//         // Assuming API returns { token: '...', user: {...} } or similar
//         // Or your API might return token in data: { token: '...', user: {...} }
//         const token = response.token || response.data?.token;
//         let userData = response.user || response.data?.user;

//         if (!token) {
//             throw new Error(response.message || "Login failed: No token received from server.");
//         }

//         await AsyncStorage.setItem(USER_TOKEN_KEY, token);

//         // If user data is not directly in login response, fetch it
//         if (!userData) {
//           console.log("AuthContext: User data not in login response, fetching separately...");
//           userData = await apiService.getElderlyUser(token); // This might be nested under 'data' too
//           userData = userData.data || userData; // Adjust if your getElderlyUser returns { data: { user_details } }
//         }
...
//         throw error; // Re-throw for the LoginScreen to handle if needed
//       }
//     },
//     signOut: async () => {
//       dispatch({ type: 'SET_LOADING', isLoading: true });
...
//           // await apiService.logoutElderly(authState.userToken);
//         }
//       } catch (e) {
...
//       dispatch({ type: 'SET_LOADING', isLoading: true });
//       try {
//         // Assuming your register API also logs in the user or returns a token & user data
//         // If not, the user will have to log in manually after registration.
//         const response = await apiService.registerElderly({ name, email, password, password_confirmation });
//         Alert.alert('Registration Successful', response.message || 'Please login with your new credentials.');
//         dispatch({ type: 'SET_LOADING', isLoading: false });
//         // Potentially navigate to Login screen or auto-login if API supports it
//       } catch (error) {
...
//         throw error; // Re-throw for the RegisterScreen
//       }
//     },
...
//                 updatedUserData = updatedUserData.data || updatedUserData; // Adjust if nested
//                 await AsyncStorage.setItem(USER_DATA_KEY, JSON.stringify(updatedUserData));
//                 dispatch({ type: 'SET_USER_DATA', userData: updatedUserData });
//                 dispatch({ type: 'SET_LOADING', isLoading: false });
//             } catch (error) {
//                 console.error("AuthContext: Failed to fetch/update user data", error);
//                 dispatch({ type: 'SET_LOADING', isLoading: false });
//                 // If token is invalid (e.g., 401 Unauthenticated), sign out the user
//                 if (error.message.includes("Unauthenticated") || error.status === 401) {
//                     await authContextValue.signOut(); // Call the memoized signOut
//                 }
//             }
//         }
//     }
//   }), [authState]); // authContextValue depends on authState.
...
//           userData = JSON.parse(userDataString);
//         }
//         console.log("AuthContext: Restored from storage - Token:", userToken, "UserData:", userData);
//       } catch (e) {
//         console.error('AuthContext: Restoring token/data failed', e);
//       }
...
//     try {
//       const device_name = `${Device.osName} - ${Device.modelName || 'Device'}`;
//       console.log('AuthContext: Signing in with payload:', { email, password_length: password?.length, device_name });
//       const response = await apiService.loginElderly({ email, password, device_name });
...
//       if (!userData && token) {
//         console.log("AuthContext: User data not in login response, fetching separately...");
//         const userResponse = await apiService.getElderlyUser(token);
...
//       console.error('AuthContext: Sign In Error -', errorMessage, error);
...
//         // Placeholder values for additional required fields defined directly here
...
//       console.log('AuthContext: Signing up with payload:', JSON.stringify(registrationPayload, null, 2));
